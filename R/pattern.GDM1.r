pattern.GDM1<-function(data,performanceVariable,scaleType="i",nomOptValues=NULL,weightsType="equal",weights=NULL,normalization="n0",patternType="upper",patternCoordinates="dataBounds",patternManual=NULL,nominalTransfMethod=NULL){
data<-as.matrix(data)
pdata<-data

normalizations<-paste("n",0:11,sep="")
...
if(length(scaleType)==1)scaleType<-rep(scaleType,ncol(data))
if (length(scaleType)!=ncol(data)) stop("scale types vector should have the size equal to number of variables")
for(v in scaleType){
  if(sum(v==c("r","i"))==0) stop("scale type should be one of the following: r-ratio i - interval")
}
...
if (patternCoordinates=="manual" && length(patternManual)!=ncol(data)) stop("pattern value vector should have the size equal to number of variables")

if(sum(performanceVariable=="n")==0)nominalTransfMethod<-rep("q",ncol(data))
...
for(v in nominalTransfMethod){
  if(sum(v==c("q","d"))==0) stop("transfer methods should be one of the following: q-quotient,d-difference")
}
if(sum(normalization==normalizations)!=1)stop(paste("Normalization method should be one of the following:", paste(normalizations,collapse=",")))



#PRZEKSZTALCENIE NOMINANT NA STYMULANTY
for(i in 1:ncol(data)){
  if(performanceVariable[i]=="n"){
    if(nominalTransfMethod[i]=="q"){
      if(scaleType[i]=="i")stop("Quotient transfer method should be used only for ratio data")
      for(j in 1:nrow(data)){
        data[j,i]<-min(data[j,i],nomOptValues[i])/max(data[j,i],nomOptValues[i])
      }
    }
    else{
...
if(sum(scaleType=="i")!=0  && sum(normalization==paste("n",6:11,sep=""))!=0)stop(paste("Normalization ",normalization," method should not be used for interval data"))

dataAndPattern<-data.Normalization(rbind(data,pattern),normalization)
...
#print("GDM distances form pattern object")
#print(gdm_p)
...
#WYKRESY
resul<-list(pdata=pdata,tdata=tdata,data=ndata,distances=gdm_p,sortedDistances=sortedgdm_p)
resul
