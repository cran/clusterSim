    ...
    if (is.vector(x) && !is.list(x)) {
        if (is.numeric(resul <- x)) 
            resul <- switch(type, n0 = x, n1 = (x - mean(x))/sd(x), 
                ...
                n6 = x/sd(x), n6a = x/mad(x), n7 = x/(max(x) - min(x)),
                n8 = x/max(x), n9 = x/mean(x), n9a = x/median(x), 
                n10 = x/sum(x), n11 = x/(sum(x^2)^0.5))
            ...
            for (nn in names(x)) {
                if (is.numeric(x[, nn])) {
                  resul <- switch(type, n0 = cbind(resul, (x[, 
                    ...
                    nn] - median(x[, nn]))/(mad(x[, nn]))), 
                    n3 = cbind(resul, (x[, nn] - mean(x[, nn]))/(max(x[, nn]) - 
                      min(x[, nn]))), n3a = cbind(resul, (x[, nn] - median(x[, nn]))/(max(x[, nn]) - 
                      min(x[, nn]))), n4 = cbind(resul, (x[, 
                    nn] - min(x[, nn]))/(max(x[, nn]) - min(x[, 
                    nn]))), n5 = cbind(resul, (x[, nn] - mean(x[, 
                    nn]))/(max(abs(x[, nn] - mean(x[, nn]))))), 
                    n5a = cbind(resul, (x[, nn] - median(x[, 
                    nn]))/(max(abs(x[, nn] - median(x[, nn]))))), 
                    n6 = cbind(resul, (x[, nn])/sd(x[, nn])),
                    n6a = cbind(resul, (x[, nn])/mad(x[, nn])), 
                    n7 = cbind(resul, (x[, nn])/(max(x[, nn]) - 
                      min(x[, nn]))), n8 = cbind(resul, (x[, 
                      nn])/(max(x[, nn]))), n9 = cbind(resul, 
                      (x[, nn])/(mean(x[, nn]))), n9a = cbind(resul, (x[, nn])/(median(x[, nn]))),
                      n10 = cbind(resul, (x[, nn])/(sum(x[, nn]))), n11 = cbind(resul, 
                      (x[, nn])/(sum(x[, nn]^2)^0.5)))
                }
                else {
            ...
            for (nn in 1:nrow(x)) {
                if (sum(is.na(as.numeric((x[nn, ])))) == 0) {
                  resul <- switch(type, n0 = rbind(resul, (x[nn, 
                    ...
                    (x[nn, ] - median(x[nn, ]))/(mad(as.numeric(x[nn, 
                    ...
                    ])))/(max(abs(x[nn, ] - mean(as.numeric(x[nn, 
                    ])))))), n5a = rbind(resul, (x[nn, ] - median(as.numeric(x[nn, 
                    ])))/(max(abs(x[nn, ] - median(as.numeric(x[nn, ])))))), 
                    ...
                    (x[nn, ])/(mean(as.numeric(x[nn, ])))),
                     n9a = rbind(resul, (x[nn, ])/(median(as.numeric(x[nn, ])))),
                     n10 = rbind(resul, (x[nn, ])/(sum(as.numeric(x[nn, ])))), n11 = rbind(resul, 
                    (x[nn, ])/(sum(as.numeric(x[nn, ])^2)^0.5)))
                }
                else {
            ...
            if (bycolumn) {
                for (i in 1:ncol(x)) resul <- switch(type, n0 = cbind(resul, 
                  ...
                  (x[, i] - mean(x[, i]))/(max(x[, i]) - min(x[, 
                    i]))), n3a = cbind(resul, (x[, i] - median(x[, i]))/(max(x[, i]) - min(x[, 
                    i]))), n4 = cbind(resul, (x[, i] - min(x[, 
                  i]))/(max(x[, i]) - min(x[, i]))), n5 = cbind(resul, 
                  (x[, i] - mean(x[, i]))/(max(abs(x[, i] - mean(x[, 
                    i]))))), n5a = cbind(resul, (x[, i] - median(x[, i]))/(max(abs(x[, i] - median(x[, 
                    i]))))), n6 = cbind(resul, (x[, i])/sd(x[, 
                  i])), n6a = cbind(resul, (x[, i])/mad(x[, i])),
                  n7 = cbind(resul, (x[, i])/(max(x[, i]) - 
                ...
                for (i in 1:nrow(x)) resul <- switch(type, n0 = rbind(resul, 
                  ...
                  ]))/(max(x[i, ]) - min(x[i, ]))), n5 = rbind(resul, 
                  (x[i, ] - mean(x[i, ]))/(max(abs(x[i, ] - mean(x[i, 
        ...
        for (i in 1:length(x)) if (is.numeric(resul[[i]] <- x[[i]])) 
            resul[[i]] <- switch(type, n0 = x[[i]], n1 = (x[[i]] - 
                ...
                n8 = x[[i]]/(max(xx)), n9 = x[[i]]/(mean(xx)), 
                n9a = x[[i]]/(median(xx)),
                n10 = x[[i]]/(sum(xx)), n11 = x[[i]]/(sum(xx^2)^0.5))
        else warning("Data not numeric, normalization not applicable")
    }
