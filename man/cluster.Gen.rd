...
\item{means}{matrix of cluster means (e.g. \code{means=matrix(c(0,8,0,8),2,2)}). If \code{means = NULL} matrix should be read from \emph{means_<modelNumber>.csv file}}
...
\emph{cov_<modelNumber>.csv file}.
...
\emph{means_<modelNumber>.csv} and  covariance matrix for all clusters should be read 
from \emph{cov_<modelNumber>.csv} and if  \code{fixedCov=FALSE} \code{means} should be read from 

\emph{means_<modelNumber>.csv} 
and covariance matrices should be read separately for each cluster 
from \emph{cov_<modelNumber>_<clusterNumber>.csv}}
...
\emph{means_<modelNumber>.csv} and \emph{cov_<modelNumber>.csv} files}
\item{inputHeader}{\code{inputHeader=TRUE} indicates that input files (\emph{means_<modelNumber>.csv};

\emph{cov_<modelNumber...>.csv}) contain header row}
\item{inputRowNames}{\code{inputRowNames=TRUE} indicates that input files (\emph{means_<modelNumber>.csv}; \emph{cov_<modelNumber...>.csv}) contain first column with row names or with number of objects (positive integer values)}
